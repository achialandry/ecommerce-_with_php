<?php 
	 ...
	 $post_parent = '';
	   ...
	   if(isset($_GET['edit'])&& !empty($_GET['edit'])){
			...
			$edt_sql = "SELECT * FROM categories WHERE id='$edit_id' ";
			$edit_result = $db -> query($edt_sql);
	 ...
	 if(isset($_GET['delete'])&& !empty($_GET['delete'])){
			...
			$sql3 ="SELECT * FROM categories WHERE id ='$delete_id' ";
			$result2 = $db ->query($sql3);
			$category2 = mysqli_fetch_assoc($result2);
			if($category2['parent'] ==0){
				$sql4 = "DELETE FROM categories WHERE parent ='$delete_id' ";
				$db ->query($sql4);
			}
	 ...
	 if(isset($_POST) && !empty($_POST)){
			...
			if(isset($_GET['edit'])){
					$id = $edit_category['id'];
					//checking $id to make sure category and id don't match
			...
			if ($count >0){
				$errors[] .= $category. ' already exists, please enter a new category';
				}
		 ...
		 //trying to get parent details match up while editing or modifying admin info
		 $category_value='';
		 $parent_value = 0;
		 if(isset($_GET['edit'])){
    ...
    <div class="row ">
        ...
        <div class="col-sm-6">
            <form class="form" action="categories.php <?= ((isset($_GET['edit']))?'edit='.$edit_id:'');?>" method="post">
                ...
                <!--Printing out the error first on top of form if any error is found from query right above-->
